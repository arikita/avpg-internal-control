...
webRoutes.get('/app', async (c) => {
  ...
  const [mgrRow, bodRow, enRow, pendMgr, pendBod, pendEn] = await Promise.all([
    ...
    c.env.DB.prepare(
      ...
      .bind(email)
      .first<{ n: number }>(),
    // BOD inbox count: general (manager_approved) + PR (en_approved nếu cần EN, manager_approved nếu không).
    c.env.DB.prepare(
...
webRoutes.get('/p/:id{[0-9]+}', async (c) => {
  ...
  if (proposal.proposal_type === 'purchase' && proposal.status === 'completed') {
    ...
    const att = await c.env.DB.prepare(
      ...
      .bind(id)
      .all<Record<string, unknown>>();
    procurement = { head: head ?? null, events: ev.results ?? [], attachments: att.results ?? [] };
  }
  return c.html(
